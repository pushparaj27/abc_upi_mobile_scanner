...
public class SwiftABCUPIMobileScannerPlugin: NSObject, FlutterPlugin {
    
    /// The mobile scanner object that handles all logic
    private let mobileScanner: ABCUPIMobileScanner
    ...
    private static func isBarcodeInScanWindow(barcode: Barcode, imageSize: CGSize) -> Bool {
        let scanwindow = SwiftABCUPIMobileScannerPlugin.scanWindow!
    ...
    init(barcodeHandler: BarcodeHandler, registry: FlutterTextureRegistry) {
        self.mobileScanner = ABCUPIMobileScanner(registry: registry, mobileScannerCallback: { barcodes, error, image in
            if barcodes != nil {
                let barcodesMap: [Any?] = barcodes!.compactMap { barcode in
                    if (SwiftABCUPIMobileScannerPlugin.scanWindow != nil) {
                        if (SwiftABCUPIMobileScannerPlugin.isBarcodeInScanWindow(barcode: barcode, imageSize: image.size)) {
                            return barcode.data
                        } else {
                            return nil
                        }
    ...
    public static func register(with registrar: FlutterPluginRegistrar) {
        let instance = SwiftABCUPIMobileScannerPlugin(barcodeHandler: BarcodeHandler(registrar: registrar), registry: registrar.textures())
        let methodChannel = FlutterMethodChannel(name:
                                                    "dev.adityabirlacapital.abc_upi_mobile_scanner/scanner/method", binaryMessenger: registrar.messenger())
    ...
    func updateScanWindow(_ call: FlutterMethodCall, _ result: @escaping FlutterResult) {
        ...
        SwiftABCUPIMobileScannerPlugin.scanWindow = scanWindowData
